...
#define IR_DATA PIND.3
#define L_STT   PORTC.2
#define L1      PORTC.3
#define L2      PORTC.4
#define L3      PORTC.5
eeprom unsigned char k1,k2,k3;
...
unsigned char IR_GetCode(void)
{
    unsigned char i;
    unsigned char Data;
    unsigned long int data_infr=0;
    if(IR_CheckStart())             // Neu dung xung START
    {
        ...
        {    
            ...
            if(IR_DATA==1) data_infr|=1;
            while(IR_DATA==1);      // cho canh len
            while(IR_DATA==0);     
        }
        Data=data_infr<<4;
        delay_ms(100);               // Cho het tin hieu (chong nhieu) 
    }
...
// State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=P Bit2=T Bit1=0 Bit0=T 
